...
class LifeLone_MNIST(object):
    ...
    def Train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            batch = Xtest[0:32]
            batch = np.concatenate((batch, fashion_Xtest[0:32]), axis=0)
            x_batch = np.reshape(batch,(-1,28,28,1))
            reco = self.GiveReconstruction(x_batch)
            reco = reco * 255.0
            x_batch = x_batch * 255.0
            cv2.imwrite(os.path.join("results/", 'OnlineVAEExpandClear_MNISTtoFashion_real.png'), merge2(x_batch[:64], [8, 8]))
            cv2.imwrite(os.path.join("results/", 'OnlineVAEExpandClear_MNISTtoFashion_reco.png'), merge2(reco[:64], [8, 8]))
